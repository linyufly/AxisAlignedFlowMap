...
TetrahedralGrid::TetrahedralGrid(lcsUnstructuredGridWithTimeVaryingPointData *grid, bool first, int id) {
	if (!grid) return;
	
	this->numOfVertices = grid->GetNumberOfPoints();
	this->numOfCells = grid->GetNumberOfCells();
	...
	for (int i = 0; i < this->numOfVertices; i++) {
		...
		//grid->GetPointData()->GetVectors()->GetTuple(i, velocity);
		grid->GetTimeVaryingPointData(id)->GetVectors()->GetTuple(i, velocity);
		this->velocities[i] = Vector(velocity);
	}
